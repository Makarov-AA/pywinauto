...
qt_quick_samples_folder = os.path.abspath(os.path.join(
    os.path.dirname(__file__), "..", "..", "apps", "Qt5_quick_samples_linux"))
qt_quick_mousearea_app = os.path.join(qt_quick_samples_folder, "mousearea")
qt_quick_view_comparison_app = os.path.join(qt_quick_samples_folder, "qquickviewcomparison")
...
class Qt5QuickMouseAreaWrapperLinuxTests(unittest.TestCase):
    """Wrapper tests for the Qt5 Quick mousearea example."""

    def setUp(self):
        _set_timings()
        self.app = Application(backend="qt5").start(qt_quick_mousearea_app)
        time.sleep(2)
...
class Qt5QuickViewComparisonWrapperLinuxTests(unittest.TestCase):
    """Wrapper tests for the mixed Qt5 QWidget/Qt Quick example."""

    def setUp(self):
        _set_timings()
        self.app = Application(backend="qt5").start(qt_quick_view_comparison_app)
        time.sleep(2)
