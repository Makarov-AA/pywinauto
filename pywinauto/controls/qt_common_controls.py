# -*- coding: utf-8 -*-
"""Version-neutral Qt control wrapper behavior."""

from __future__ import unicode_literals

from ..qt_common.rect import RECT


class CommonWindowWrapper(object):

    """Common behavior for Qt top-level windows."""
...
class CommonPaneWrapper(object):

    """Common behavior for generic Qt container controls."""

    pass


class CommonButtonWrapper(object):

    """Common behavior for Qt buttons, check boxes, and radio buttons."""
...
class CommonEditWrapper(object):

    """Common behavior for Qt edit controls."""

    has_title = False

    # TODO: implement selection/text cursor APIs.
    ...
    def set_window_text(self, text, append=False):
        """Set edit text, optionally appending to current value."""
        if append:
...
class CommonComboBoxWrapper(object):

    """Common behavior for Qt comboboxes."""
    ...
    def is_editable(self):
        """Return whether combobox is editable."""
        return bool(self.get_native_property('editable'))


class CommonTabControlWrapper(object):

    """Common behavior for Qt tab controls."""

    # TODO: add TabItem wrappers.
...
class CommonSliderWrapper(object):

    """Common behavior for Qt slider, scrollbar, spinbox, and progress controls."""
...
class CommonListViewWrapper(object):

    """Common behavior for Qt list controls."""

    # TODO: return item wrappers of all children instead of only visible children.
...
class CommonTreeViewWrapper(object):

    """Common behavior for Qt tree controls."""
...
class CommonTableWrapper(object):

    """Common behavior for Qt table controls."""
    ...
    def click(self, row=None, column=None):
        """Click table or a cell addressed by row and column."""
        if row is None and column is None:
            return super(CommonTableWrapper, self).click()
